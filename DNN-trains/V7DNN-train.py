...
# Config
# ── L_SELFACT + L_OVERSHOOT EXCLUSION TEST ───────────────────────────────
# Building on the L_selfact-only exclusion (which raised val latent acc
# 48.8% -> 56.5%). L_overshoot is now the lowest-accuracy remaining class
# (0.46 val) and touches 3 of the 5 flagged confused pairs, though it is
# NOT isolated the way L_selfact was — the remaining classes form a more
# evenly-spread mutual confusion cluster, so a smaller gain than the
# first exclusion is the honest expectation here, not a repeat 8-point jump.
EXCLUDED_LATENT_LABELS = {1, 3}  # L_selfact, L_overshoot (original numbering)
LABEL_REMAP = {0: 0, 2: 1, 4: 2, 5: 3, 6: 4}  # original -> contiguous (5 classes)

N_LATENT_CLASSES   = 5
N_COUPLING_CLASSES = 4
NULLCLINE_DIM      = 49
XCORR_DIM          = 10
ALL_NEW_DIM        = 88
V6_NEW_DIM         = 12
FP_MULTI_DIM       = 10
BRANCH1C_DIM       = ALL_NEW_DIM + V6_NEW_DIM + FP_MULTI_DIM
...
DATA_ROOT   = r'C:/Users/nickj/LTInetV7 Local Data Drive'
...
latent_names   = ['L_mirna', 'L_gk',
                   'L_bistable', 'L_delay', 'NULL']  # L_selfact, L_overshoot excluded
...
for sys_i in range(1, N_SYS + 1):
    for lat in range(6):
        if lat in EXCLUDED_LATENT_LABELS:
            continue  # skip L_selfact and L_overshoot folders entirely
        for ch in range(3):
            d = f'{DATA_ROOT}/V7Lat{lat}_Ch{ch}_Sys{sys_i}_NNdata'
            data_dirs.append(d)
            sys_idx_lookup[d] = sys_i - 1
    d_null = f'{DATA_ROOT}/V7Lat6_Ch3_Sys{sys_i}_NNdata'
    data_dirs.append(d_null)
    sys_idx_lookup[d_null] = sys_i - 1


class LTInetV7Dataset(Dataset):
    def __init__(self, data_dirs, sys_idx_lookup, nullcline_dim):
        ...
        for d in data_dirs:
            ...
            for fp in files:
                try:
                    ...
                    if sl in EXCLUDED_LATENT_LABELS:
                        continue  # defensive: skip even if it somehow got loaded
                    if sl not in LABEL_REMAP:
                        continue
                    sl = LABEL_REMAP[sl]
                    ...
                    anf_full = np.concatenate([anf, v6n, fpm])
        ...
        if skipped_no_anf > 0:
            print(f'  [INFO] {skipped_no_anf} files missing all_new_features - run V7_annex_full.m first.')
...
class PreloadedTensorDataset(Dataset):
    """
    ~500MB total is trivial against a 6GB GPU (RTX PRO 500). Stacking every
    sample into a few large tensors and moving THOSE to the GPU once, up
    front, avoids per-batch CPU->GPU transfer and per-sample __getitem__
    overhead entirely. Shuffling is done manually via GPU-side index
    permutation each epoch instead of relying on DataLoader.
    """
...
class LTInetV7_NoSelfactNoOvershoot(nn.Module):
    def __init__(self):
        super().__init__()
...
if __name__ == '__main__':
    ...
    dataset = LTInetV7Dataset(data_dirs, sys_idx_lookup, NULLCLINE_DIM)
    if len(dataset) == 0:
        raise RuntimeError('No valid examples found. Check data_dirs and run V7_annex_full.m first.')
    ...
    model = LTInetV7_NoSelfactNoOvershoot().to(device)
    ...
    SAVE_PATH = f'{WEIGHTS_DIR}/LTInetV7_NoSelfactNoOvershoot_best.pth'
    best_avg_acc = 0.0
    n_epochs = 200
    es_patience = 60
    no_improve = 0
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if va_avg > best_avg_acc:
            ...
            np.save(f'{WEIGHTS_DIR}/V7_NoSelfactNoOvershoot_nc_mean.npy', nc_mean.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V7_NoSelfactNoOvershoot_nc_std.npy', nc_std.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V7_NoSelfactNoOvershoot_xc_mean.npy', xc_mean.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V7_NoSelfactNoOvershoot_xc_std.npy', xc_std.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V7_NoSelfactNoOvershoot_anf_mean.npy', anf_mean.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V7_NoSelfactNoOvershoot_anf_std.npy', anf_std.cpu().numpy())
        else:
            no_improve += 1
        ...
        if epoch % 20 == 0:
            ...
            fig.suptitle(f'LTInetV7-NoSelfactNoOvershoot | Epoch {epoch} | '
