...
DATA_ROOT   = r'C:/Users/nickj/LTInetV8 Local Data Drive'
...
# V8 finalized latent library — Subset A from V8_Trimmed8_LDA.py greedy
# search (min pairwise CV acc 0.686, every pair MODERATE-or-better, no
# WEAK/DEGENERATE remaining). Indices 0-5 real classes, 6 = null.
...
for sys_i in range(1, N_SYS + 1):
    for lat in range(6):        # L_mirna .. L_overshoot
        for ch in range(3):     # Ch0-2
            d = f'{DATA_ROOT}/V8Lat{lat}_Ch{ch}_Sys{sys_i}_NNdata'
            data_dirs.append(d)
            sys_idx_lookup[d] = sys_i - 1
    d_null = f'{DATA_ROOT}/V8Lat6_Ch3_Sys{sys_i}_NNdata'
    data_dirs.append(d_null)
    sys_idx_lookup[d_null] = sys_i - 1


class LTInetV8Dataset(Dataset):
...
class PreloadedTensorDataset(Dataset):
    """
    Same VRAM-preload approach as LTInetV7.py: dataset is small enough
...
if __name__ == '__main__':
    ...
    dataset = LTInetV8Dataset(data_dirs, sys_idx_lookup, NULLCLINE_DIM)
    if len(dataset) == 0:
    ...
    SAVE_PATH = f'{WEIGHTS_DIR}/LTInetV8_full_best.pth'
    best_avg_acc = 0.0
    n_epochs = 500
    es_patience = 60
    no_improve = 0
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if va_avg > best_avg_acc:
            ...
            np.save(f'{WEIGHTS_DIR}/V8_nc_mean.npy', nc_mean.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V8_nc_std.npy', nc_std.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V8_xc_mean.npy', xc_mean.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V8_xc_std.npy', xc_std.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V8_anf_mean.npy', anf_mean.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V8_anf_std.npy', anf_std.cpu().numpy())
        else:
            no_improve += 1
        ...
        if epoch % 20 == 0:
            ...
            fig.suptitle(f'LTInetV8 | Epoch {epoch} | '
