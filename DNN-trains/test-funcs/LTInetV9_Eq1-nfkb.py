...
# ================================================================
# ABLATION: Eq1-ONLY, FULL CLASS SET (7 latent incl. NULL, 4 coupling incl. Ch3)
#
# Follow-up to LTInetV9_Eq1Only.py (which excluded NULL/Ch3 for a clean
# 6-vs-3-class isolation test). That version confirmed the y-equation
# regime is not intrinsically harder to learn (recovered to ~80%/91%,
# matching V8's Eq0 baseline) — but it wasn't directly comparable to
# V8's full 7-class/4-coupling structure, and it couldn't express "no
# latent needed" (NULL) at all.
#
# This version restores NULL and Ch3 so the class structure exactly
# matches V8/full-V9, while still training on Eq1 data ONLY (no Eq0
# folders) — giving a same-class-count, same-coupling-count comparison
# against V8's validated 82.9% latent / 91.7% coupling result, isolating
# the "which equation" variable as cleanly as the class structure allows.
#
# NULL class data is EQUATION-AGNOSTIC by construction (V9_ProductionGen.m
# generates it once per system with no Eq split, since coupling is
# undefined for null) — so the same NULL folders used in V8/full-V9 are
# reused here unchanged; only the real (latent, coupling) folders are
# restricted to Eq1.
# ================================================================

# Config
N_LATENT_CLASSES   = 7    # L_mirna, L_gk, L_yhill, L_bistable, L_delay, L_overshoot, NULL
...
# Cap on how many files are loaded per (latent, coupling, system) folder.
# Applies equally to the NULL folder. Tune directly for sweeps.
MAX_FILES_PER_CONFIG = 100
...
# V9 finalized latent library x 3 real coupling channels, EQ1 ONLY
# (coupling -> dy), PLUS the equation-agnostic NULL class. Eq0 folders
# are deliberately excluded from data_dirs.
latent_names   = ['L_mirna', 'L_gk', 'L_yhill', 'L_bistable',
                   'L_delay', 'L_overshoot', 'NULL']
coupling_names = ['Ch0 HillRep', 'Ch1 Additive', 'Ch2 Multiplicative', 'Ch3 None']

data_dirs = []
sys_idx_lookup = {}
for sys_i in range(1, N_SYS + 1):
    for lat in range(6):        # L_mirna .. L_overshoot
        for ch in range(3):     # Ch0-2
            d = f'{DATA_ROOT}/V9Lat{lat}_Ch{ch}_Eq1_Sys{sys_i}_NNdata'
            data_dirs.append(d)
...
class PreloadedTensorDataset(Dataset):
    ...
    def __init__(self, dataset, device):
        ...
        for i in range(n):
            nc, xc, anf, xi, topo, sl, cl, sys_id, sys_name = dataset[i]
...
class LTInetV9(nn.Module):
    ...
    head_latent:   128->96->48->7   (L_mirna, L_gk, L_yhill, L_bistable, L_delay, L_overshoot, NULL)
    head_coupling: 128->64->32->4   (Ch0-2 + Ch3 null)
    No head_eq: this dataset is Eq1-only for all non-null examples;
    NULL examples have no eq at all. Either way there is no eq axis to
    classify here, unlike the full pooled V9 model.
    """
    def __init__(self):
        super().__init__()
...
def train_epoch(model, preloaded_train, optimizer, loss_fn_lat, loss_fn_coup, device,
                 nc_mean, nc_std, xc_mean, xc_std, anf_mean, anf_std, batch_size=64):
    model.train()
    total_loss = lat_correct = coup_correct = total = 0
    for nc, xc, anf, xi, topo, slbl, clbl, _, _ in iterate_batches(preloaded_train, batch_size, True, device):
        if nc.shape[0] < 2:
            continue
        ...
        optimizer.zero_grad()
        ll, lc = model(nc_n, xc_n, anf_n, xi, topo)
    ...
    return total_loss / n_batches, lat_correct / max(total, 1), coup_correct / max(total, 1)


def evaluate(model, preloaded_val, loss_fn_lat, loss_fn_coup, device,
             nc_mean, nc_std, xc_mean, xc_std, anf_mean, anf_std, batch_size=64):
    model.eval()
    total_loss = lat_correct = coup_correct = total = 0
    with torch.no_grad():
        for nc, xc, anf, xi, topo, slbl, clbl, _, _ in iterate_batches(preloaded_val, batch_size, False, device):
            nc_n = (nc - nc_mean) / nc_std
            xc_n = (xc - xc_mean) / xc_std
            anf_n = (anf - anf_mean) / anf_std
            ll, lc = model(nc_n, xc_n, anf_n, xi, topo)
...
def evaluate_per_system(model, preloaded_val, device,
    ...
    with torch.no_grad():
        for nc, xc, anf, xi, topo, slbl, clbl, sys_id, sys_name in iterate_batches(preloaded_val, batch_size, False, device):
            nc_n = (nc - nc_mean) / nc_std
            xc_n = (xc - xc_mean) / xc_std
            anf_n = (anf - anf_mean) / anf_std
            ll, lc = model(nc_n, xc_n, anf_n, xi, topo)
            lat_pred = ll.argmax(1).cpu(); coup_pred = lc.argmax(1).cpu()
            slbl_cpu = slbl.cpu(); clbl_cpu = clbl.cpu()
...
if __name__ == '__main__':
    ...
    SAVE_PATH = f'{WEIGHTS_DIR}/LTInetV9_Eq1Only_FullClasses_best.pth'
    best_avg_acc = 0.0
    n_epochs = 500
    es_patience = 60
    no_improve = 0
    ...
    for epoch in range(1, n_epochs + 1):
        tr_loss, tr_lat, tr_coup = train_epoch(
            model, preloaded_train, optimizer, loss_fn_lat, loss_fn_coup, device,
            nc_mean, nc_std, xc_mean, xc_std, anf_mean, anf_std, BATCH_SIZE)
        va_loss, va_lat, va_coup = evaluate(
            model, preloaded_val, loss_fn_lat, loss_fn_coup, device,
            nc_mean, nc_std, xc_mean, xc_std, anf_mean, anf_std, BATCH_SIZE)
        ...
        if va_avg > best_avg_acc:
            ...
            np.save(f'{WEIGHTS_DIR}/V9_Eq1Only_FullClasses_nc_mean.npy', nc_mean.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V9_Eq1Only_FullClasses_nc_std.npy', nc_std.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V9_Eq1Only_FullClasses_xc_mean.npy', xc_mean.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V9_Eq1Only_FullClasses_xc_std.npy', xc_std.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V9_Eq1Only_FullClasses_anf_mean.npy', anf_mean.cpu().numpy())
            np.save(f'{WEIGHTS_DIR}/V9_Eq1Only_FullClasses_anf_std.npy', anf_std.cpu().numpy())
        else:
            no_improve += 1
        ...
        if epoch % 20 == 0:
            ...
            fig.suptitle(f'LTInetV9 Eq1-Only [Full Classes: 7 lat, 4 coup] | Epoch {epoch} | '
