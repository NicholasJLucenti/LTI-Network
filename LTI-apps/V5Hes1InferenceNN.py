...
# ── Paths ─────────────────────────────────────────────────────────────────
LTI_ROOT     = r'C:/Users/nickj/MATLAB Drive/Compiled Works/LTI Network'
MAT_PATH     = f'{LTI_ROOT}/Hes1_V5_inference.mat'
WEIGHTS_PATH = f'{LTI_ROOT}/LTInetV5_full_best.pth'
NORM_ROOT    = LTI_ROOT
...
# ── Model definition — must exactly match LTInetV5.py trainer ─────────────
...
class LTInetV5(nn.Module):
    ...
    def __init__(self):
        ...
        self.branch1c = Branch1c_AllNewMLP(88,32)
...
if 'all_new_features' not in data:
    raise KeyError(
        "'all_new_features' not found in the .mat file. "
        "Re-run Hes1_V5_inference_generator.m to produce it.")

anf_raw = data['all_new_features'].squeeze()
assert anf_raw.shape[0] == ALL_NEW_DIM, \
    f'Expected {ALL_NEW_DIM} all_new_features, got {anf_raw.shape[0]}'
anf = torch.tensor(np.clip(anf_raw,-50,50).astype(np.float32)).unsqueeze(0)
...
# ── Load normalisation ────────────────────────────────────────────────────
try:
    nc_mean  = torch.tensor(np.load(f'{NORM_ROOT}/V5_nc_mean.npy'),  dtype=torch.float32)
    nc_std   = torch.tensor(np.load(f'{NORM_ROOT}/V5_nc_std.npy'),   dtype=torch.float32)
    xc_mean  = torch.tensor(np.load(f'{NORM_ROOT}/V5_xc_mean.npy'),  dtype=torch.float32)
    xc_std   = torch.tensor(np.load(f'{NORM_ROOT}/V5_xc_std.npy'),   dtype=torch.float32)
    anf_mean = torch.tensor(np.load(f'{NORM_ROOT}/V5_anf_mean.npy'), dtype=torch.float32)
    anf_std  = torch.tensor(np.load(f'{NORM_ROOT}/V5_anf_std.npy'),  dtype=torch.float32)
    nc_std[nc_std < 1e-6]   = 1.0
    xc_std[xc_std < 1e-6]   = 1.0
    anf_std[anf_std < 1e-6] = 1.0
except FileNotFoundError as e:
    raise FileNotFoundError(
        f'V5 normalisation .npy files not found in {NORM_ROOT}. '
        'These save automatically during training on best val acc. '
        f'Missing: {e}')
...
lat_mean   = lat_probs.mean(dim=0);  lat_std  = lat_probs.std(dim=0)
coup_mean  = coup_probs.mean(dim=0); coup_std = coup_probs.std(dim=0)

lat_entropy  = (-lat_mean  * torch.log(lat_mean  + 1e-8)).sum().item()
coup_entropy = (-coup_mean * torch.log(coup_mean + 1e-8)).sum().item()

# ── Print results ─────────────────────────────────────────────────────────
print(f'\nLTInetV5 Hes1 Inference')
