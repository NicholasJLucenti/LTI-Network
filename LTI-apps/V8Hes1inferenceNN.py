...
from LTInetV8 import (
    LTInetV8, N_LATENT_CLASSES, N_COUPLING_CLASSES,
    NULLCLINE_DIM, XCORR_DIM, ALL_NEW_DIM, V6_NEW_DIM, FP_MULTI_DIM,
    latent_names, coupling_names, topology_map, WEIGHTS_DIR
)
...
coupling_descriptions = {
    ...
    'Ch2 Multiplicative': 'Multiplicative: -gamma_c * I * x',
...
# ── Normalization ─────────────────────────────────────────────────────
try:
    nc_mean  = torch.tensor(np.load(f'{WEIGHTS_DIR}/V8_nc_mean.npy'),  dtype=torch.float32)
    nc_std   = torch.tensor(np.load(f'{WEIGHTS_DIR}/V8_nc_std.npy'),   dtype=torch.float32)
    xc_mean  = torch.tensor(np.load(f'{WEIGHTS_DIR}/V8_xc_mean.npy'),  dtype=torch.float32)
    xc_std   = torch.tensor(np.load(f'{WEIGHTS_DIR}/V8_xc_std.npy'),   dtype=torch.float32)
    anf_mean = torch.tensor(np.load(f'{WEIGHTS_DIR}/V8_anf_mean.npy'), dtype=torch.float32)
    anf_std  = torch.tensor(np.load(f'{WEIGHTS_DIR}/V8_anf_std.npy'),  dtype=torch.float32)
    nc_std[nc_std < 1e-6] = 1.
    xc_std[xc_std < 1e-6] = 1.
    anf_std[anf_std < 1e-6] = 1.
except FileNotFoundError as e:
    raise FileNotFoundError(f'V8 norm files not found in {WEIGHTS_DIR}. Missing: {e}')

# Sanity check: raw Hes1 features vs training normalization stats.
# If Hes1's real-data features fall wildly outside the range the
# normalization was fit to (synthetic data), that's a meaningful
# out-of-distribution warning worth surfacing before trusting any
# probability the model outputs.
...
model.load_state_dict(torch.load(f'{WEIGHTS_DIR}/LTInetV8_full_best.pth', map_location=device))
model.eval()
...
# ── Results ───────────────────────────────────────────────────────────
print(f'\nLTInetV8 Hes1 Inference (real experimental data)')
...
print('\n-- Joint probability table (Latent x Coupling) --')
