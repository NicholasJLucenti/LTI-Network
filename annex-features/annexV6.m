...
for fi = 1:length(v6_folders)
    ...
    for mf = 1:length(mat_files)
        ...
        if isfield(mat,'annex_v6_complete')
        ...
        for k = 2:n_all
            ...
            phi = v6_annex_lib_row(xk, yk, hill_k0, hill_km, hill_n, libid, hill_rep, hill_deg);
        ...
        phase_mean_dx = zeros(N_BINS,1); phase_std_dx = zeros(N_BINS,1);
        ...
        phase_mean_dy = zeros(N_BINS,1); phase_std_dy = zeros(N_BINS,1);
        ...
        if ~all(isfinite(crqa_features)); crqa_features = zeros(7,1); end

        wavelet_features = compute_wavelet_energy(x_all, y_all, dt, N_SCALES);
        if ~all(isfinite(wavelet_features)); wavelet_features = zeros(12,1); end

        poincare_features = compute_poincare(x_all, y_all);
        if ~all(isfinite(poincare_features)); poincare_features = zeros(6,1); end
        ...
        all_new_features = [phase_x_features; phase_y_features; amp_features; ...
                            rqa_features; crqa_features; wavelet_features; ...
        ...
        annex_v6_complete = true;
        annex_used_fallback_hill_params = used_fallback;

        save(fpath, 'all_new_features', 'rqa_phase_features', ...
             'annex_v6_complete', 'annex_used_fallback_hill_params', '-append');
...
function phi = v6_annex_lib_row(x, y, hill_k0, hill_km, hill_n, libid, hill_rep, hill_deg)
    switch libid
        case 'brusselator'
            phi=[1,x,x^2,y,x^2*y,hill_rep(y,hill_k0,hill_n),hill_deg(y,hill_km)];
        case 'fhn'
            phi=[1,x,x^3,y,hill_rep(y,hill_k0,hill_n),hill_deg(y,hill_km),x*y,x^2];
        case 'rma'
            phi=[1,x,x^2,y,x*y,x^2*y,hill_rep(y,hill_k0,hill_n),hill_deg(y,hill_km)];
        case 'toggle'
            phi=[1,x,y,hill_rep(y,hill_k0,hill_n),hill_rep(x,hill_k0,hill_n),hill_deg(y,hill_km)];
        otherwise
            phi=[1,x,x^2,x^3,y,y^2,y^3,hill_rep(y,hill_k0,hill_n),hill_deg(y,hill_km)];
    end
end
...
function rqa = compute_rqa(x, rr_target, min_line)
    ...
    rqa = max(min([actual_RR; DET; LAM; min(TT,50); min(ENTR,10); min(L_mean,50); DIV], 100), 0);
end
...
function crqa = compute_crqa(x, y, rr_target, min_line)
    ...
    crqa = max(min([CRR; CDET; CLAM; min(CTT,50); min(CENTR,10); min(CL_mean,50); CDIV], 100), 0);
end
...
function wf = compute_wavelet_energy(x, y, dt, n_scales)
    ...
    energy_x = zeros(n_scales,1); energy_y = zeros(n_scales,1);
    t_vec = (0:N-1)*dt;
    for s = 1:n_scales
        sigma = scales(s) / (2*pi);
        for k = 1:N
            psi = exp(-(t_vec - t_vec(k)).^2 / (2*sigma^2)) .* cos(2*pi*(t_vec - t_vec(k)) / scales(s));
...
function pf = compute_poincare(x, y)
    ...
    n = length(y_norm); mu = mean(y_norm); sigma = std(y_norm);
    if sigma < 1e-10
        skew = 0; kurt = 0;
    else
    ...
    pf = max(min([mean(y_norm); std(y_norm); skew; kurt; period_mean; period_cv], 10), -10);
end