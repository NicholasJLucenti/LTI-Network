function [ok, pack] = V6_ProcessFeatures(x_data, y_data, xa, ya, N, n_all, dt, ...
    sg_p, sg_f, p, sysdef, ridge_lambda, sparsity_thresh, n_stridge_iters)

    ok = false; pack = struct();
    libid = sysdef.libid;

    [Theta_full, col_names] = V6_BuildLib(x_data, y_data, N, p, libid);
    ...
    [nullcline_features, ok_nc, fp_extra] = v6_nullcline_features_internal( ...
    ...
    for k=2:n_all
        row = V6_LibRow(xa(k-1),ya(k-1),p,libid);
        if length(row)==size(Xi,1)
            dx_model(k)=row*Xi(:,1);
        end
    end
    resid_dx = dx_obs - dx_model;
    xcorr_features = v6_compute_xcorr_internal(resid_dx, xa, ya);
    ...
    elseif size(Xi_ternary,1)>9
        Xi_ternary=Xi_ternary(1:9,:);
    end
    ...
    fnn_feat  = V6_FNN(x_data, dt);
    te_feat   = V6_TransferEntropy(resid_dx, x_data, y_data);
    spec_feat = V6_ResidualSpectrum(resid_dx, dt);
    v6_new_features = [fnn_feat; te_feat; spec_feat];
    if ~all(isfinite(v6_new_features))
        pack.fail_reason = 'v6_new_features_nonfinite'; return;
    end

    pack.nullcline_features = nullcline_features;
    pack.xcorr_features     = xcorr_features;
    pack.Xi_ternary          = Xi_ternary;
    pack.Xi                  = Xi;
    pack.topology            = topology;
    pack.x_data_all          = xa;
    pack.y_data_all          = ya;
    pack.t_ode               = (0:dt:(n_all-1)*dt)';
    pack.col_names           = col_names;
    pack.v6_new_features     = v6_new_features;
...
function [feat, ok, fp_extra] = v6_nullcline_features_internal(x_data, y_data, N, p, libid, Xi, tSx, tSy, is_multi_fp)
    ok=false; feat=zeros(49,1); fp_extra=zeros(10,1);
    ng=50; xg=linspace(min(x_data),max(x_data),ng); yg=linspace(min(y_data),max(y_data),ng);
    [XG,YG]=meshgrid(xg,yg);
    Phi=V6_LibGrid(XG(:),YG(:),p,libid);
    ...
    Phi_o=V6_LibGrid(x_data,y_data,p,libid);
    ...
    if is_multi_fp
        [fp_extra, ~] = V6_MultiFixedPoint(dX, dY, xg, yg);
    end
end
...
function xcf=v6_compute_xcorr_internal(rdx,xa,ya)
    ml=50;
