...
% ── CONFIG ────────────────────────────────────────────────────────────────
% V9 annexation is IDENTICAL in logic to V8's — every feature block below
% (phase-resolved residuals, RQA, cross-RQA, wavelet energy, Poincare,
% SINDy coefficient magnitudes) is already computed symmetrically from
% x_data_all/y_data_all/Xi and has no dependency on which equation the
% coupling term was spliced into. Only DATA_ROOT and the completion-flag
% field name changed to avoid silently treating V8-annexed files as done.
...
for i = 1:length(folders)
    if contains(folders(i).name,'NNdata')
        v9_folders{end+1} = fullfile(DATA_ROOT, folders(i).name);
    end
end

fprintf('Found %d V9 folders\n', length(v9_folders));
total_processed = 0; total_skipped = 0;
t_overall_start = tic;

for fi = 1:length(v9_folders)
    folder_path = v9_folders{fi};
    mat_files   = dir(fullfile(folder_path,'*.mat'));
    fprintf('\n[%d/%d] %s — %d files\n', fi, length(v9_folders), folder_path, length(mat_files));
    t_folder_start = tic;
    ...
    for mf = 1:length(mat_files)
        ...
        if isfield(mat,'annex_v91_complete')
        ...
        %% ── SINDY RESIDUALS — VECTORIZED ─────────────────────────────────
        x_lag = [0; x_all(1:end-1)];
        y_lag = [0; y_all(1:end-1)];

        Theta_full = v9_build_lib_vectorized(x_lag, y_lag, hill_k0, hill_km, hill_n, libid, hill_rep, hill_deg);
        ...
        annex_v91_complete  = true;

        save(fpath, 'all_new_features', 'rqa_phase_features', ...
             'annex_v91_complete', '-append');
...
%% ── HELPER: VECTORIZED library matrix builder (unchanged from V7/V8) ────
function Theta = v9_build_lib_vectorized(x, y, hill_k0, hill_km, hill_n, libid, hill_rep, hill_deg)
    n = length(x);
...
%% ── HELPER FUNCTIONS (unchanged from V6/V7/V8) ────────────────────────────
