...
n_examples   = 500;
save_dir = 'C:\Users\nickj\MATLAB Drive\LTI Network\Lat1_Sys1_NNdata';
...
while saved < n_examples
    ...
    var_resid_x = var(resid_dx);
    var_resid_y = var(resid_dy);

    if var_resid_x < 2 * var_resid_y
        skipped = skipped + 1;
        continue
    end
    ...
    is_closed = loop_err < 0.15;
    seg       = round(length(x_2nd)/3);
    amp_ratio = std(x_2nd(end-seg+1:end)) / max(std(x_2nd(1:seg)), 1e-8);

    if     amp_ratio > 1.15;  amp_trend = 'growing';
    elseif amp_ratio < 0.85;  amp_trend = 'decaying';
    ...
    save(fname, ...
        ...
        'structure_label', ...
        'params_saved', ...
        'x_data_all', 'y_data_all', 'z_data_all', ...
        't_ode', 'Xi');
