...
n_examples = 500;
save_dir   = 'C:/Users/nickj/MATLAB Drive/LTI Network/Lat3_Sys2_NNdata';
...
while saved < n_examples
    ...
    av      = av_range(1)      + diff(av_range)      * rand();
    bv      = bv_range(1)      + diff(bv_range)      * rand();
    cv      = cv_range(1)      + diff(cv_range)      * rand();
    ...
    % Brusselator base — Structure class 2 latent
    % dx/dt = a - (b+1)*x + x^2*y - H_rep(I)*x
    % dy/dt = b*x - x^2*y
    % dI/dt = av*x - bv*x*I - cv*I    [coupled degradation — class 2]
    ode_rhs = @(t, s) [ ...
        a - (b+1)*s(1) + s(1)^2*s(2) - hill_rep(s(3), hill_k0, hill_n)*s(1); ...
        b*s(1) - s(1)^2*s(2);                                                  ...
        av*s(1) - bv*s(1)*s(3) - cv*s(3) ];

    opts_ode = odeset('RelTol',1e-9,'AbsTol',1e-11);

    try
        [t_ode, S] = ode45(ode_rhs, t_span, [a; b/a; 0.5], opts_ode);
    catch
        skipped = skipped + 1;
        continue
    end
    ...
    params_saved = struct( ...
        ...
        'hill_km', hill_km, 'av',      av,      ...
        'bv',      bv,      'cv',      cv,      ...
        'noise',   noise,   'system',  'brusselator');
