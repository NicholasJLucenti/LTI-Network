...
n_examples = 500;
save_dir   = 'C:/Users/nickj/MATLAB Drive/LTI Network/Lat3_Sys3_NNdata';
if ~exist(save_dir, 'dir'), mkdir(save_dir); end

rng(5);

hill_rep = @(y, k, n)  k^n ./ (k^n + y.^n);
hill_deg = @(x, km)    x   ./ (km  + x);
...
while saved < n_examples
    ...
    delta   = delta_range(1)  + diff(delta_range)  * rand();
    km      = km_range(1)     + diff(km_range)     * rand();
    ...
    hill_k  = hill_k_range(1) + diff(hill_k_range) * rand();
    a       = a_range(1)      + diff(a_range)      * rand();
    b       = b_range(1)      + diff(b_range)      * rand();
    c       = c_range(1)      + diff(c_range)      * rand();
    noise   = noise_range(1)  + diff(noise_range)  * rand();

    t_end  = (N + 200) * dt;
    t_span = 0 : dt : t_end;

    % Repressilator base — Structure class 2 latent
    % dx/dt = alpha * H_rep(I) - delta * H_deg(x)
    % dy/dt = beta * x - gamma * y
    % dI/dt = a*x - b*x*I - c*I     [coupled degradation — class 2]
    ode_rhs = @(t, s) [ ...
        alpha * hill_rep(s(3), hill_k, hill_n) - delta * hill_deg(s(1), km); ...
        beta  * s(1) - gamma * s(2);                                          ...
        a * s(1) - b * s(1) * s(3) - c * s(3) ];

    opts_ode = odeset('RelTol',1e-9,'AbsTol',1e-11);

    try
        [t_ode, S] = ode45(ode_rhs, t_span, [1.0; 0.5; 0.5], opts_ode);
    catch
        skipped = skipped + 1;
        continue
    end
    ...
    if var_z_tail < 1e-6 || var_x_tail < 1e-6
        skipped = skipped + 1;
        continue
    end
    ...
    params_saved = struct( ...
        'alpha',  alpha,  'delta', delta, 'km',     km,    ...
        'beta',   beta,   'gamma', gamma, 'hill_n', hill_n, ...
        'hill_k', hill_k, 'a',     a,     'b',      b,     ...
        'c',      c,      'noise', noise, ...
...
function [h_val, h_var] = aux_x(xi_drain, var_drain, ...
        colscale_full, targetScaleX, hill_k, hill_n, x_data, y_data)
    ...
    h_basis_avg = mean(hill_k^hill_n ./ (hill_k^hill_n + y_data.^hill_n));
...
function [h_val, h_var] = aux_y(xi_prod, var_prod, ...
        colscale_full, targetScaleY, km, x_data, y_data)
    ...
    h_basis_avg = mean(y_data ./ (km + y_data));
