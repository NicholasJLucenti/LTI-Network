...
for sys_idx = 1:4
    ...
    save_dir = sprintf( ...
        'C:/Users/nickj/MATLAB Drive/LTI Network/Lat4_Sys%d_NNdata', ...
        sys_idx);

    if ~exist(save_dir, 'dir'), mkdir(save_dir); end

    fprintf('\n=== Latent Class 4 | System: %s ===\n', system_name);
    ...
    switch system_name
        ...
        case 'repressilator'
            param_sampler = @() struct( ...
                ...
                'lb', {{[0,-inf,-inf,-inf,0,0,0],[0,0,0,0,-inf,-inf,-inf]}}, ...
                'ub', {{[0,0,0,0,0,0,0],[0,inf,inf,inf,0,0,0]}});
            ode_fun = @(t,s,p) repressilator_ode(t,s,p);
        ...
        case 'van_der_pol'
            ...
            ode_fun = @(t,s,p) vanderpol_ode(t,s,p);
    ...
    run_generation(system_name, ode_fun, ic_fun, param_sampler, ...
        lib_fun, aux_fun, latent_class, n_per_system, save_dir, ...
        sys_idx*10 + 43);

end

fprintf('\nAll Latent Class 4 datasets complete.\n');

%% ── GENERATION LOOP ──────────────────────────────────────────────────────
function run_generation(system_name, ode_fun, ic_fun, param_sampler, ...
        lib_fun, aux_fun, latent_class, n_examples, save_dir, rng_seed)
...
while saved < n_examples
    ...
    if mod(saved,50)==0
        fprintf('[%s | Lat4] Saved %d/%d  (skipped %d, attempts %d)\n', ...
            system_name, saved, n_examples, skipped, attempt);
    end
end

fprintf('[%s | Lat4] Done. Saved %d, skipped %d, attempts %d\n', ...
    system_name, saved, skipped, attempt);
end

%% ── ODE FUNCTIONS ────────────────────────────────────────────────────────
function ds = goodwin_ode(~, s, p)
    ds = zeros(3,1);
    ds(1) = p.alpha * p.hill_rep(s(3),p.hill_k0,p.hill_n) - p.d1*s(1);
    ds(2) = p.ks*s(1) - p.Vmax * p.hill_deg(s(2),p.hill_km);
    ds(3) = p.kp*s(2) - p.kd*s(3);
end

function ds = brusselator_ode(~, s, p)
    ds = zeros(3,1);
...
function ds = repressilator_ode(~, s, p)
    ds = zeros(3,1);
...
%% ── BRUSSELATOR PARAM SAMPLER ────────────────────────────────────────────
function p = sample_brusselator(hill_rep, hill_deg)
    while true
