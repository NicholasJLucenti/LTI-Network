...
fprintf('\nAll V2 additional datasets complete.\n');
...
while saved < n_examples
    ...
    %% ── FFT COMPUTATION ──────────────────────────────────────────────
    % Pad or truncate residuals to n_fft points
    ...
    % Magnitude spectra normalized to max
    mag_x = abs(Fx);
    mag_y = abs(Fy);
    mag_x = mag_x / (max(mag_x) + 1e-8);
    mag_y = mag_y / (max(mag_y) + 1e-8);

    % Cross-spectral phase difference
    phase_diff = angle(Fx .* conj(Fy));

    % Assemble 3 x n_bins FFT feature matrix
