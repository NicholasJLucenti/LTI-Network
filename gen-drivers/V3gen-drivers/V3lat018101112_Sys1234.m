...
sg_f         = 11;
base_seed    = 77;
n_per_config = 200;   % additional files per folder on top of existing

% Active latent classes for the x-driven 6-class model
% Lat0:  linear x           dI/dt = kp*x - kd*I
% Lat1:  Hill activation x  dI/dt = kp*H_act(x,ka,na) - kd*I
% Lat8:  MM degradation     dI/dt = kp*x - kcat*I/(Km+I)
% Lat10: Quadratic x        dI/dt = kp*x^2 - kd*I
% Lat11: Incoherent FF      dI/dt = kp*x*H_rep(x,k0,n) - kd*I
% Lat12: Delayed linear x   dI/dt = kp*x_filt - kd*I  (4-state ODE)
...
for sys_idx = 1:length(systems)
    ...
    for lc_idx = 1:length(active_latents)
        ...
        % Lat12 uses a 4-state ODE; all others use 3-state
...
fprintf('\n\nAll V3 Lat0,1,8,10,11,12 × Sys1-4 generation complete.\n');
...
rng(rng_seed);
% Pick up from however many files already exist in the folder
...
while saved < n_examples
    ...
    %% ── XCORR FEATURES ───────────────────────────────────────────────
    % Recompute resid_dx from Xi and observables (not saved separately)
    ...
    xcorr_features = compute_xcorr_features(resid_dx, x_data_all, y_data_all);

    if any(~isfinite(xcorr_features))
        skipped = skipped+1; continue
    end
...
%% ── ODE FUNCTIONS — 3-STATE (Lat0,1,8,10,11) ────────────────────────────
...
% State: [x, y, I, x_filt]  where x_filt approximates delayed x
% dx_filt/dt = (x - x_filt) / tau_d    (first-order filter)
% dI/dt = kp*x_filt - kd*I
...
function p = sample_goodwin(hill_rep, hill_deg, hill_act, latent_class)
    p = struct( ...
        ...
        'kcat',    2+4*rand(),        ...
        'Km',      0.5+2.0*rand(),    ...
        'tau_d',   1.5+2.0*rand(),    ...   % min 1.5 for measurable lag
...
function feat = nullcline_shape_features(xpts, ypts)
    ...
    feat=[cx;cy;pca_angle;aspect;curv_feat;asym_along;asym_across;
...
function xcorr_features = compute_xcorr_features(resid_dx, x_data_all, y_data_all)
    ...
    [pv_x,pi_x]=max(abs(xc));
    pl_x  = lags_x(pi_x);
    pw_x  = sum(abs(xc)>pv_x/2);
    pos_x = mean(abs(xc(lags_x>0)));
    neg_x = mean(abs(xc(lags_x<0)));
    la_x  = (pos_x-neg_x)/(pos_x+neg_x+1e-8);
    cz_x  = xc(lags_x==0);

    [yc,lags_y]=xcorr(r_norm,y_norm,max_lag,'normalized');
    [pv_y,pi_y]=max(abs(yc));
    pl_y  = lags_y(pi_y);
    pw_y  = sum(abs(yc)>pv_y/2);
    pos_y = mean(abs(yc(lags_y>0)));
    neg_y = mean(abs(yc(lags_y<0)));
    la_y  = (pos_y-neg_y)/(pos_y+neg_y+1e-8);
    cz_y  = yc(lags_y==0);

    xcorr_features = [pl_x;pv_x;pw_x;la_x;cz_x; ...
                      pl_y;pv_y;pw_y;la_y;cz_y];
end
