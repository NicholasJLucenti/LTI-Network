...
% ── Three new systems ─────────────────────────────────────────────────────
%
% Sys5 — FitzHugh-Nagumo (FHN)
%   dx/dt = x - x³/3 - y + I_ext + C(I,x)
%   dy/dt = (x + a_fhn - b_fhn*y) / tau_fhn
%   Topological signature: cubic x-nullcline, linear y-nullcline,
%   fast-slow timescale separation. Only system that can operate in
%   EXCITABLE regime (single pulse per perturbation) as well as
%   oscillatory — directly models p53 pulse dynamics.
%
% Sys6 — Activator-inhibitor with self-activation (Tyson-Goldbeter type)
%   dx/dt = s*x²/(K²+x²) - d*x - r*y*x/(J+x) + C(I,x)
%   dy/dt = a_ai*x - b_ai*y
%   Topological signature: S-shaped (fold) x-nullcline from Hill
%   self-activation. Produces oscillations via SUBCRITICAL Hopf or
%   SNIC — the bifurcation type documented for p53 in Ciliberto 2005
%   and the IET Systems Biology review. No current system has a fold.
%
% Sys7 — Rosenzweig-MacArthur predator-prey
%   dx/dt = r_rm*x*(1-x/K_rm) - a_rm*x*y/(b_rm+x) + C(I,x)
%   dy/dt = e_rm*a_rm*x*y/(b_rm+x) - d_rm*y
%   Topological signature: hump-shaped x-nullcline (logistic growth),
%   VERTICAL y-nullcline. No current system has a vertical y-nullcline.
%   Maps directly to p53 (prey) / Mdm2 (predator) with Holling Type II
%   saturation in ubiquitination kinetics.

systems_new  = {'fitzhugh_nagumo', 'activator_inhibitor', 'rosenzweig_macarthur'};
sys_offsets  = [5, 6, 7];   % Sys5, Sys6, Sys7 folder indices

active_latents    = [0, 1, 2, 3, 4];
coupling_channels = [0, 1, 2];

for s_idx = 1:length(systems_new)
    system_name = systems_new{s_idx};
    ...
    for lc_idx = 1:length(active_latents)
        ...
        for ch_idx = 1:length(coupling_channels)
            ...
            save_dir = sprintf( ...
                ...
                latent_class, coupling_ch, sys_folder);
            ...
            switch system_name
                case 'fitzhugh_nagumo'
                    param_sampler = @() sample_fhn(hill_rep,hill_deg, ...
                        hill_act,latent_class);
                    ode_fun = @(t,s,p) fhn_ode(t,s,p,latent_class,coupling_ch);
                    ic_fun  = @(p) [0.5; 0.1; 0.5];
                    lib_fun = @(x,y,N_,p) standard_lib(x,y,N_,p);

                case 'activator_inhibitor'
                    param_sampler = @() sample_ai(hill_rep,hill_deg, ...
                        hill_act,latent_class);
                    ode_fun = @(t,s,p) ai_ode(t,s,p,latent_class,coupling_ch);
                    ic_fun  = @(p) [p.K_ai; p.a_ai*p.K_ai/(2*p.b_ai); 0.5];
                    lib_fun = @(x,y,N_,p) standard_lib(x,y,N_,p);

                case 'rosenzweig_macarthur'
                    param_sampler = @() sample_rm(hill_rep,hill_deg, ...
                        hill_act,latent_class);
                    ode_fun = @(t,s,p) rm_ode(t,s,p,latent_class,coupling_ch);
                    ic_fun  = @(p) [p.x_eq*1.1; p.y_eq*0.9; 0.5];
                    lib_fun = @(x,y,N_,p) rm_lib(x,y,N_,p);
            end

            run_generation(system_name, ode_fun, ic_fun, param_sampler, ...
                lib_fun, latent_class, coupling_ch, n_per_config, save_dir, ...
                base_seed + sys_folder*1000 + latent_class*10 + coupling_ch);
        end
    end
end

fprintf('\nAll V4 Sys5/6/7 generation complete.\n');
...
while saved < n_examples
    ...
    % Rosenzweig-MacArthur: reject if prey goes extinct
    if strcmp(system_name,'rosenzweig_macarthur')
        if min(x_data_all)<0 || min(y_data_all)<0
            skipped=skipped+1; continue; end
    end
...
% Cubic x-nullcline (x - x³/3 = y + const), linear y-nullcline
% tau_fhn controls slow/fast separation — large tau = more relaxation-like
function ds=fhn_ode(~,s,p,latent_class,coupling_ch)
    ds=zeros(3,1);
    x=s(1); y=s(2); I=s(3);
    ds(1) = x - (x^3)/3 - y + p.I_ext + coupling_term(I,x,p,coupling_ch);
    ds(2) = (x + p.a_fhn - p.b_fhn*y) / p.tau_fhn;
    ds(3) = latent_z(s,p,latent_class);
end

% Sys6: Activator-inhibitor with Hill self-activation
% S-shaped x-nullcline from s*x²/(K²+x²) self-activation competing
% with d*x linear degradation and r*y*x/(J+x) y-mediated degradation.
% Produces subcritical Hopf / SNIC oscillations.
function ds=ai_ode(~,s,p,latent_class,coupling_ch)
    ds=zeros(3,1);
    x=s(1); y=s(2); I=s(3);
    self_act = p.s_ai * x^2 / (p.K_ai^2 + x^2);
    y_deg    = p.r_ai * y * x / (p.J_ai + x);
    ds(1) = self_act - p.d_ai*x - y_deg + coupling_term(I,x,p,coupling_ch);
    ds(2) = p.a_ai*x - p.b_ai*y;
    ds(3) = latent_z(s,p,latent_class);
end

% Sys7: Rosenzweig-MacArthur predator-prey
% Hump-shaped x-nullcline (logistic growth minus Holling Type II predation)
% Vertical y-nullcline at x* = b_rm*d_rm/(e_rm*a_rm - d_rm)
function ds=rm_ode(~,s,p,latent_class,coupling_ch)
    ds=zeros(3,1);
    x=max(s(1),0); y=max(s(2),0); I=s(3);  % positivity guard
    pred = p.a_rm*x*y/(p.b_rm+x);
    ds(1) = p.r_rm*x*(1-x/p.K_rm) - pred + coupling_term(I,x,p,coupling_ch);
    ds(2) = p.e_rm*pred - p.d_rm*y;
    ds(3) = latent_z(s,p,latent_class);
end
...
%% ── PARAM SAMPLERS ───────────────────────────────────────────────────────
function p=sample_fhn(hill_rep,hill_deg,hill_act,~)
    p=struct( ...
        'a_fhn',   0.5+0.5*rand(),    ...  % excitability threshold
        'b_fhn',   0.5+0.3*rand(),    ...  % recovery coupling
        'tau_fhn', 8+12*rand(),       ...  % slow timescale [8,20]
        'I_ext',   0.35+0.55*rand(),  ...  % external drive — determines oscillatory/excitable
        'hill_n',  round(2+3*rand()), 'hill_k0',1+2*rand(), ...
        'hill_km', 0.3+1.2*rand(),    ...
        'alpha_c', 0.5+1.5*rand(),    'beta_c',0.3+1.2*rand(), ...
        'gamma_c', 0.05+0.15*rand(),  ...  % small for FHN stability
...
function p=sample_ai(hill_rep,hill_deg,hill_act,~)
    % Oscillation requires self-activation to overcome linear degradation
    % near x=K_ai: s_ai/2 > d_ai is a necessary condition
    while true
        s_ai = 3+5*rand();
        K_ai = 0.5+1.5*rand();
        d_ai = 0.3+0.9*rand();
        if s_ai/2 > d_ai; break; end
    end
    p=struct( ...
        's_ai',  s_ai,            ...  % self-activation strength
        'K_ai',  K_ai,            ...  % self-activation threshold
        'd_ai',  d_ai,            ...  % linear degradation
        'r_ai',  1+3*rand(),      ...  % y-mediated degradation rate
        'J_ai',  0.1+0.9*rand(),  ...  % MM constant for y-degradation
        'a_ai',  1+3*rand(),      ...  % y production rate (by x)
        'b_ai',  0.5+2.0*rand(),  ...  % y degradation rate
        'hill_n', round(2+3*rand()), 'hill_k0',1+2*rand(), ...
        'hill_km',0.3+1.2*rand(),    ...
        'alpha_c',0.5+1.5*rand(),    'beta_c',0.3+1.2*rand(), ...
        'gamma_c',0.1+0.3*rand(),    ...
        'kp',     1+2*rand(),        'kd',   0.5+1.5*rand(), ...
        'km',     0.5+1.5*rand(),    'I_max',2+4*rand(), ...
        'kcat',   2+5*rand(),        'Km',   0.3+1.5*rand(), ...
        'hill_rep',hill_rep,'hill_deg',hill_deg,'hill_act',hill_act, ...
...
function p=sample_rm(hill_rep,hill_deg,hill_act,~)
    % Enforce coexistence and oscillation conditions for RM:
    %   x_eq = b_rm*d_rm/(e_rm*a_rm - d_rm) > 0  → e_rm*a_rm > d_rm
    %   y_eq > 0  → x_eq < K_rm
    %   Hopf condition (approximately): K_rm > b_rm + 2*x_eq
    while true
        r_rm  = 0.5+1.5*rand();
        K_rm  = 4+6*rand();
        a_rm  = 1+3*rand();
        ...
        e_rm  = 0.3+0.5*rand();
        d_rm  = 0.1+0.4*rand();
        if e_rm*a_rm <= d_rm; continue; end
        x_eq = b_rm*d_rm/(e_rm*a_rm - d_rm);
        if x_eq >= K_rm || x_eq <= 0; continue; end
        y_eq = r_rm*(1-x_eq/K_rm)*(b_rm+x_eq)/a_rm;
        if y_eq <= 0; continue; end
        % Hopf bifurcation approximately when K_rm > b_rm + 2*x_eq
        if K_rm > b_rm + 2*x_eq; break; end
    end
    p=struct( ...
        'r_rm',  r_rm,  'K_rm',  K_rm,  'a_rm',  a_rm, ...
        'b_rm',  b_rm,  'e_rm',  e_rm,  'd_rm',  d_rm, ...
        'x_eq',  x_eq,  'y_eq',  y_eq,  ...  % precomputed for IC
        'hill_n', round(2+3*rand()), 'hill_k0',1+2*rand(), ...
        'hill_km',0.3+1.2*rand(),   ...
        'alpha_c',0.5+1.5*rand(),   'beta_c',0.3+1.2*rand(), ...
        'gamma_c',0.1+0.3*rand(),   ...
...
%% ── LIBRARY BUILDERS ─────────────────────────────────────────────────────
% FHN and Activator-inhibitor use the standard 9-term library.
% RM uses a 7-term library with x*y cross-term replacing x³,
% since the Holling Type II predation term x*y/(b+x) is better
% approximated by x*y than by x³.
...
function lib=rm_lib(x,y,N_,p)
    % Cross-term x*y captures Holling Type II predation x*y/(b+x)
    lib.Theta_full=[ones(N_,1),x,x.^2,y,x.*y, ...
                   p.hill_rep(y,p.hill_k0,p.hill_n),p.hill_deg(y,p.hill_km)];
    lib.Theta_poly=lib.Theta_full(:,1:5);
    lib.col_names={'1','x','x^2','y','xy','HillRep','HillDeg'};
end
...
function Phi=lib_matrix_grid(x,y,p,system_name)
    ...
    if strcmp(system_name,'rosenzweig_macarthur')
        Phi=[ones(n,1),x,x.^2,y,x.*y,hr,hd];
    else
