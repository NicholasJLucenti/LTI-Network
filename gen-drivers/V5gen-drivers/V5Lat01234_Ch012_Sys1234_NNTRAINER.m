...
% STRidge parameters — fixed globally, no per-file tuning
ridge_lambda    = 0.05;
sparsity_thresh = 0.02;
n_stridge_iters = 10;

% Informativeness filter thresholds
min_rel_amp          = 0.08;  % oscillation must be >=8% of trajectory mean scale
min_transient_ratio   = 0.3;  % first-half var must be >=30% of second-half var

% ── V5 design (same latents and channels as before) ────────────────────────
...
excluded = [4,0,1; 3,0,1; 0,1,3; 0,2,3; 1,1,3; 2,2,3; 3,1,3; 3,2,3; 1,1,1];
...
for sys_idx = 1:length(systems)
    ...
    for lc_idx = 1:length(active_latents)
        ...
        for ch_idx = 1:length(coupling_channels)
            ...
            save_dir = sprintf( ...
                'C:/Users/nickj/MATLAB Drive/LTI Network/V5Lat%d_Ch%d_Sys%d_NNdata', ...
                latent_class, coupling_ch, sys_idx);
            if ~exist(save_dir,'dir'), mkdir(save_dir); end

            fprintf('\n=== STRidge V5 | %s | Lat%d | Ch%d ===\n', ...
            ...
            run_generation(system_name, ode_fun, ic_fun, param_sampler, ...
                ...
                ridge_lambda, sparsity_thresh, n_stridge_iters, ...
                min_rel_amp, min_transient_ratio);
        end
    end
end

fprintf('\nAll STRidge V5 Lat0-4 x Ch0-2 x Sys1-4 generation complete.\n');
...
while saved < n_examples
    ...
    amp_x_window  = max(x_data) - min(x_data);
    mean_x_window = mean(abs(x_data)) + 1e-8;
    rel_amp_x     = amp_x_window / mean_x_window;
    if rel_amp_x < min_rel_amp
        skipped=skipped+1; continue;
    end

    first_half_var  = var(x_data(1:round(N/2)));
    second_half_var = var(x_data(round(N/2):N));
    if first_half_var < min_transient_ratio * second_half_var
        skipped=skipped+1; continue;
    end
    ...
    if targetScaleX < 1e-8 || targetScaleY < 1e-8
        skipped=skipped+1; continue;
    end
    ...
    for eq = 1:2
        Theta_eq = ThetaN_full;
        y_eq     = dSdtN(:,eq);
        active   = true(n_terms,1);

        for iter = 1:n_stridge_iters
            Theta_active = Theta_eq(:,active);
            A = Theta_active'*Theta_active + ridge_lambda*eye(sum(active));
            b = Theta_active'*y_eq;
            try
                coef_active = A \ b;
            catch
                fit_failed = true; break;
            end

            coef_full = zeros(n_terms,1);
            coef_full(active) = coef_active;
            new_active = abs(coef_full) >= sparsity_thresh;

            if isequal(new_active,active)
                active = new_active; break;
            end
            active = new_active;
            if ~any(active)
                active = true(n_terms,1); break;
            end
        end
        if fit_failed; break; end
    ...
    if fit_failed || any(~isfinite(XiN(:)))
        skipped=skipped+1; continue;
    end
    ...
    params_saved    = rmfield_safe(p,{'hill_rep','hill_deg','hill_act'});
    params_saved.system        = system_name;
    params_saved.coupling_ch   = coupling_ch;
    params_saved.sindy_method  = 'STRidge';
    params_saved.ridge_lambda  = ridge_lambda;
