...
% ================================================================
% V8 PRODUCTION GENERATOR
% Finalized 6-class latent library, validated via V8_ClusterSearch_LDA.py
% + V8_Trimmed8_LDA.py (Subset A: min pairwise CV acc 0.686, all pairs
% MODERATE-or-better, no WEAK/DEGENERATE pairs remaining).
%
...
dt = 0.05; N = 300; sg_p=3; sg_f=11; n_per_config = 250; base_seed = 40260;
...
latent_names_v8 = {'L_mirna','L_gk','L_yhill','L_bistable','L_delay','L_overshoot'};
coupling_channels = [0, 1, 2];

S = V6_SystemLib();
n_systems = length(S);

fprintf('=== V8 Production Generation: %d systems x %d latents x %d couplings + null ===\n', ...
    n_systems, length(active_latents), length(coupling_channels));
fprintf('Latent library: %s\n', strjoin(latent_names_v8, ', '));
...
for sys_idx = 1:n_systems
    ...
    for lat = active_latents
        ...
        for ch = coupling_channels
            ...
            while saved < n_per_config && attempt < (n_per_config-existing_count)*40
                ...
                t_start = tic;
                odefn_full = @(t,s,p) v8_coupled_ode(t,s,p,sysdef,lat,ch, ...
    ...
    %  NULL CLASS (Lat6/Ch3)
    % ================================================================
    save_dir = sprintf('%s/V8Lat%d_Ch%d_Sys%d_NNdata', DATA_ROOT, NULL_LAT, NULL_COUP, sys_idx);
    if ~exist(save_dir,'dir'); mkdir(save_dir); end
    fprintf('\n--- %s | NULL ---\n', sysdef.name);
    ...
    while saved < n_per_config && attempt < (n_per_config-existing_count_null)*40
        ...
        try
            if sysdef.multi_fp
                ...
                odefn_forced = @(t,s,p) v8_watchdog_wrap( ...
                ...
                odefn_null = @(t,s,p) v8_watchdog_wrap( ...
...
fprintf('\n=== V8 PRODUCTION GENERATION COMPLETE ===\n');
fprintf('Total saved: %d   Total skipped: %d\n', total_saved, total_skipped);
fprintf('Run a V8-adapted annexation script next (V7_annex_full.m with\n');
fprintf('DATA_ROOT updated works as-is -- library forms unchanged from V6/V7).\n');


%% ================================================================
%  HELPER: coupled ODE (base system + latent I [+ delay relay z] + coupling)
%% ================================================================
function dsdt = v8_coupled_ode(t, s, p, sysdef, lat, ch, hill_rep, hill_act, hill_deg, t_start, max_seconds)
    if toc(t_start) > max_seconds
        error('V8:ODETimeout', 'Attempt exceeded %.0fs wall-clock limit', max_seconds);
    end
...
function dsdt_out = v8_watchdog_wrap(dsdt_in, t_start, max_seconds)
    if toc(t_start) > max_seconds
        error('V8:ODETimeout', 'Attempt exceeded %.0fs wall-clock limit', max_seconds);
    end
    dsdt_out = dsdt_in;
